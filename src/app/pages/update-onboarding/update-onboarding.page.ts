...
@Component({
  ...
  templateUrl: './update-onboarding.page.html',
  styleUrls: ['./update-onboarding.page.scss'],
})
export class UpdateOnboardingPage implements OnInit {
      ...
      if (this.regForm.value.email) {
        if (
          ...
          this.api.post_public('user/check-email-account',param).then((data:any)=>{
            if(data && data.account_exist === true){
          this.util.showError('', 'Email already exist in the database');
...
this.util.showLoading();
    ...
    this.api.post_private('user/update-profile-info',param).then((data:any)=>{
      ...
      if (data){
        this.util.hideLoading();
        // this.updateFun();
        this.util.showSuccess('',"updated successfully");
